...
const DashboardPage = () => {
    ...
    api
      ...
      .catch((loadError) => {
        ...
        if (fallbackDashboard) {
          ...
          setUser(fallbackDashboard.user);
          setError("");
          return;
        }
  ...
  const quickActions = [
    ...
    { label: "Growth Planner", to: "/planner", icon: TargetIcon, tint: "text-amber-700", bg: "bg-amber-100" },
  ...
  return (
    <motion.div initial="hidden" animate="visible" className="muse-page">
      {chatOpen ? (
        <div className="fixed inset-0 z-[80] overflow-y-auto bg-slate-950/24 p-4 backdrop-blur-sm md:p-8">
          <div className="mx-auto max-w-6xl">
            <div className="mb-4 flex justify-end">
              <button
                type="button"
                onClick={() => setChatOpen(false)}
                className="inline-flex items-center gap-2 rounded-full border border-[#d8c3ae] bg-white/90 px-4 py-2 text-sm font-medium text-slate-700 shadow-[0_18px_36px_rgba(177,146,112,0.16)]"
              >
                <XCircleIcon className="h-4 w-4" />
                Close chat
              </button>
            </div>
            <ChatbotPanel token={token} userName={user?.name || dashboard.user?.name || "Explorer"} />
          </div>
        </div>
      ) : null}

      <motion.section custom={0} variants={fadeUp} className="hero-shell muse-card muse-card-peach px-6 py-7 md:px-8" data-ambient-scene="Dashboard Hero" data-ambient-intensity="0.28">
      ...
      <div className="muse-grid-two">
        <motion.section custom={1} variants={fadeUp} className="muse-grid-three">
          {(insightCards.length
            ? insightCards
            : [
            ...
            <div key={card.title} className={`metric-tile metric-${card.tone || "neutral"} muse-card p-5`}>
        ...
        <motion.section custom={2} variants={fadeUp} className="muse-card muse-card-blue p-5">
      ...
      <motion.section custom={3} variants={fadeUp} className="space-y-6">
        <div className="grid gap-6 xl:grid-cols-[0.92fr_1.08fr]">
          <div className="space-y-6">
            ...
            <div className="muse-card p-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                {quickActions.map((action) => {
                  ...
                  return (
                    <Link key={action.label} to={action.to} className="muse-mini-card p-4 transition-transform duration-200 hover:-translate-y-1">
            ...
            <div className="muse-card muse-card-peach p-5">
            ...
            <div className="muse-card p-5">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                {(analysis.dailyTasks?.length ? analysis.dailyTasks : []).map((task) => (
                  <div key={task.title} className="muse-mini-card p-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {behaviorStats.map((item) => (
            <div key={item.label} className={`metric-tile metric-${item.tone} muse-card p-4`}>
              <div className="flex items-center justify-between">
